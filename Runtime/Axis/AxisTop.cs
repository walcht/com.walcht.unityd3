...
namespace UnityD3
{
    public class AxisTop<InputType> : Axis<InputType> where InputType : unmanaged, IFormattable
    {
        /// <summary>
        ///     Constructs a new top-oriented axis generator for the given scale. In this orientation, ticks are drawn
        ...
        public AxisTop(IScaleContinuous<InputType, float> scale, int tick_count = 6) : base()
        {
            m_scale = scale;
        ...
        public override Axis<InputType> SetTickCount(int count)
        {
            ...
            {
                float x = m_scale.F(ticks[i]);
                GameObject tick_container = new($"tick_{i}");
                tick_container.transform.SetParent(m_axis_container.transform, worldPositionStays: false);
                tick_container.transform.localPosition = new Vector3(x, 0.0f, 0.0f);
            ...
            for (int i = 0; i < m_tick_count; ++i)
            {
                ...
                tick_text.transform.localPosition = new Vector3(
                    ...
                    m_tick_size + tick_text.renderedHeight / 2,
        ...
        protected override void ConstructMainLine()
        {
            ...
            positions[0] = new Vector3(m_scale.F(x0), 0.0f, 0.0f);
            positions[1] = new Vector3(m_scale.F(x1), 0.0f, 0.0f);
