...
namespace UnityD3
{
    public class Line3D<T, D> : IGenerator3D<T, D> where T : IEnumerable<D>
    {
        private LineRenderer m_line_renderer;
        private float m_stroke_width = 0.0175f;

        public Line3D(T data, Func<D, float> x_accessor, Func<D, float> y_accessor, Func<D, float> z_accessor,
        ...
        public override IGenerator3D<T, D> Generate()
        {
        ...
        public override IGenerator3D<T, D> SetStrokeWidth(float stroke_width)
        {
        ...
        public override IGenerator3D<T, D> SetMaterial(Material mat)
        {
            m_line_renderer.material = new Material(mat);
            return this;
        }

        public override IGenerator3D<T, D> SetColor(Color color)
        {
